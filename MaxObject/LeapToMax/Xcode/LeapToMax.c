...
//  
//
//  Created by Tobias Schaeuble on 21.10.13.
//
//
...
{
	switch (x->val.a_type) {
		case A_LONG: outlet_int(x->out, atom_getlong(&x->val+5)); break;
		case A_FLOAT: outlet_float(x->out, atom_getfloat(&x->val+5)); break;
		case A_SYM: outlet_anything(x->out, atom_getsym(&x->val+5), 0, NULL); break;
		default: break;
	}
}
